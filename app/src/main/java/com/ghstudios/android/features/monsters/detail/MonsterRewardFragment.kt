...
class MonsterRewardFragment : ListFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(rank == HuntingRewardListCursorLoader.RANK_EX) viewModel.rewardEXData.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
        ...
        if(rank == HuntingRewardListCursorLoader.RANK_GOU) viewModel.rewardGOUData.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
        ...
        if(rank == HuntingRewardListCursorLoader.RANK_Z1) viewModel.rewardZ1Data.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
        if(rank == HuntingRewardListCursorLoader.RANK_Z2) viewModel.rewardZ2Data.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
        if(rank == HuntingRewardListCursorLoader.RANK_Z3) viewModel.rewardZ3Data.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
        if(rank == HuntingRewardListCursorLoader.RANK_Z4) viewModel.rewardZ4Data.observe(this, Observer<List<HuntingReward>>{this.populateRewards(it)})
    }
