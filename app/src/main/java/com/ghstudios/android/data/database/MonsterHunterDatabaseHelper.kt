...
private const val DATABASE_VERSION = 36 // …34→35: 新增狩猎笛旋律表 horn_songs+指南页面；35→36: 防具家族名 name_zh 重生成（套装名、去武器标志）
...
internal class MonsterHunterDatabaseHelper constructor(ctx: Context):
    ...
    private val TAG = "MHFZZ-DB-Helper"
    ...
    override fun preCopyDatabase(db: SQLiteDatabase) {
        ...
        try {
            val asb_set_columns = arrayOf(
                    S.COLUMN_ASB_SET_NAME, S.COLUMN_ASB_SET_RANK, S.COLUMN_ASB_SET_HUNTER_TYPE,
                    S.COLUMN_ASB_WEAPON_SLOTS, S.COLUMN_ASB_WEAPON_DECORATION_1_ID, S.COLUMN_ASB_WEAPON_DECORATION_2_ID, S.COLUMN_ASB_WEAPON_DECORATION_3_ID,
                    S.COLUMN_HEAD_ARMOR_ID, S.COLUMN_HEAD_DECORATION_1_ID, S.COLUMN_HEAD_DECORATION_2_ID, S.COLUMN_HEAD_DECORATION_3_ID,
                    S.COLUMN_BODY_ARMOR_ID, S.COLUMN_BODY_DECORATION_1_ID, S.COLUMN_BODY_DECORATION_2_ID, S.COLUMN_BODY_DECORATION_3_ID,
                    S.COLUMN_ARMS_ARMOR_ID, S.COLUMN_ARMS_DECORATION_1_ID, S.COLUMN_ARMS_DECORATION_2_ID, S.COLUMN_ARMS_DECORATION_3_ID,
                    S.COLUMN_WAIST_ARMOR_ID, S.COLUMN_WAIST_DECORATION_1_ID, S.COLUMN_WAIST_DECORATION_2_ID, S.COLUMN_WAIST_DECORATION_3_ID,
                    S.COLUMN_LEGS_ARMOR_ID, S.COLUMN_LEGS_DECORATION_1_ID, S.COLUMN_LEGS_DECORATION_2_ID, S.COLUMN_LEGS_DECORATION_3_ID,
                    S.COLUMN_TALISMAN_EXISTS, S.COLUMN_TALISMAN_TYPE, S.COLUMN_TALISMAN_SLOTS,
                    S.COLUMN_TALISMAN_DECORATION_1_ID, S.COLUMN_TALISMAN_DECORATION_2_ID, S.COLUMN_TALISMAN_DECORATION_3_ID,
                    S.COLUMN_TALISMAN_SKILL_1_ID, S.COLUMN_TALISMAN_SKILL_1_POINTS, S.COLUMN_TALISMAN_SKILL_2_ID, S.COLUMN_TALISMAN_SKILL_2_POINTS)
    ...
    private fun builderDecoration(): SQLiteQueryBuilder {
        ...
        QB.tables = S.TABLE_DECORATIONS + " AS d" +
                " LEFT OUTER JOIN " + S.TABLE_ITEMS + " AS i" + " ON " + "d." +
                S.COLUMN_DECORATIONS_ID + " = " + "i." + S.COLUMN_ITEMS_ID +
                ...
                " LEFT OUTER JOIN " + S.TABLE_ITEM_TO_SKILL_TREE + " AS its3 " +
                " ON " + "i." + S.COLUMN_ITEMS_ID + " = " + "its3." + S.COLUMN_ITEM_TO_SKILL_TREE_ITEM_ID +
                " AND " + "s2." + S.COLUMN_SKILL_TREES_ID + " != " + "its3." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID +
                " AND " + "s1." + S.COLUMN_SKILL_TREES_ID + " != " + "its3." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID +
                " LEFT OUTER JOIN " + S.TABLE_SKILL_TREES + " AS s3" +
                " ON " + "its3." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID + " = " + "s3." + S.COLUMN_SKILL_TREES_ID +

                " LEFT OUTER JOIN " + S.TABLE_ITEM_TO_SKILL_TREE + " AS its4 " +
                " ON " + "i." + S.COLUMN_ITEMS_ID + " = " + "its4." + S.COLUMN_ITEM_TO_SKILL_TREE_ITEM_ID +
                " AND " + "s3." + S.COLUMN_SKILL_TREES_ID + " != " + "its4." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID +
                " AND " + "s2." + S.COLUMN_SKILL_TREES_ID + " != " + "its4." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID +
                " AND " + "s1." + S.COLUMN_SKILL_TREES_ID + " != " + "its4." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID +
                " LEFT OUTER JOIN " + S.TABLE_SKILL_TREES + " AS s4" +
                " ON " + "its4." + S.COLUMN_ITEM_TO_SKILL_TREE_SKILL_TREE_ID + " = " + "s4." + S.COLUMN_SKILL_TREES_ID
    ...
    private fun builderGathering(): SQLiteQueryBuilder {
        ...
        projectionMap[S.COLUMN_GATHERING_SEASON] = g + "." + S.COLUMN_GATHERING_SEASON
     ...
     * ***************************** BENTO QUERIES **********************************************
     */
    private val column_item_1_name get() = localizeColumn("i1.name")
    private val column_item_2_name get() = localizeColumn("i2.name")
    /*
	 * Get all bentos
	 */

    fun queryBentos(): BentoCursor {
        //      Weapon Example
    ...
    fun queryBento(id: Long): Bento? {
        return BentoCursor(db.rawQuery("""
            ...
            WHERE i._id = ?
            GROUP BY i._id
        """, arrayOf(id.toString()))).firstOrNull { it.bento }
    }
        ...
        return BentoCursor(db.rawQuery("""
            ...
            FROM ${S.TABLE_BENTO} b
                ...
                LEFT OUTER JOIN items i2
                    ON b.ingredient2_id = i2._id
            WHERE i._id = ?
        """, arrayOf(id.toString()))).firstOrNull { listOf(it.bento) }
    }
    ...
    private val column_item_name get() = localizeColumn("i1.name")
    fun queryOffersOnItemID(id: Long): StoreOfferCursor {
        return StoreOfferCursor(db.rawQuery("""
            SELECT sl._id as offer_id, item_id, cost, store, quantity, notes, $column_item_name as item_name, i1.icon_name as item_icon_name, i1.icon_color as item_icon_color
            FROM ${S.TABLE_SHOP_LIST} sl
                LEFT OUTER JOIN items i1
                    ON sl.item_id = i1._id
            WHERE item_id = ?
        """, arrayOf(id.toString())))
    }
 ...
 */
     ...
     */
    private fun builderMonster(): SQLiteQueryBuilder {
        val m = "m"
        ...
        //Create new querybuilder
        val MT = SQLiteQueryBuilder()

        MT.tables = S.TABLE_MONSTERS + " AS m"

        MT.setProjectionMap(projectionMap)
        return MT
    }
 ...
 */
    ...
    fun queryQuestHub(hub: QuestHub): QuestCursor {
        ...
        qh.OrderBy = if(hub == QuestHub.PERMIT) "permit_monster_id" else S.COLUMN_QUESTS_SORT_ORDER
        qh.Limit = null
     ...
     * Get a specific quest based on hub
     */
    fun queryMonsterType(type: MonsterType): MonsterCursor {

        val mt = QueryHelper()
        mt.Columns = null
        mt.Table = S.TABLE_MONSTERS
        mt.Selection = "m." + S.COLUMN_MONSTERS_CLASS_TYPE + " = ?"
        mt.SelectionArgs = arrayOf(type.toString())
        mt.GroupBy = null
        mt.Having = null
        //qh.OrderBy = if(hub == QuestHub.PERMIT) "_id,permit_monster_id" else S.COLUMN_QUESTS_SORT_ORDER
        mt.OrderBy = S.COLUMN_MONSTERS_CLASS_TYPE
        mt.Limit = null

        return MonsterCursor(wrapJoinHelper(builderMonster(), mt))
    }
    ...
    private fun builderQuest(): SQLiteQueryBuilder {
        ...
        val column_sub_a_goal = localizeColumn(S.COLUMN_QUESTS_SUB_A_GOAL)
        val column_sub_b_goal = localizeColumn(S.COLUMN_QUESTS_SUB_B_GOAL)
