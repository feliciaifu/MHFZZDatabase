...
class ItemDao(val dbMainHelper: SQLiteOpenHelper) {
    ...
    private val item_columns
        get() = "_id, $column_name name, item_hid, name_ja, $column_description description, " +
    ...
    fun queryBasicItems(searchTerm: String = ""): ItemCursor {
        ...
        val typePredicate: String = "('$typeItem')"

        // return all basic items, filtered
        val filter = SqlFilter(column_name, searchTerm)
        val filterHid = SqlFilter("item_hid", searchTerm)

        return ItemCursor(db.rawQuery("""
                SELECT $item_columns
                FROM items
                WHERE type in $typePredicate
                  AND (${filter.predicate} OR ${filterHid.predicate})
                ORDER BY _id
            """, arrayOf(*filter.parameters) + arrayOf(*filterHid.parameters)
        ))
    }
    ...
    fun queryItemSearch(searchTerm: String?, includeTypes: List<ItemType> = emptyList()): ItemCursor {
        ...
        return ItemCursor(db.rawQuery("""
            SELECT $item_columns
            FROM items
            WHERE (${filter.predicate} OR ${filterHid.predicate}) AND $typePredicate
            ORDER BY _id
        """, arrayOf(*filter.parameters) + arrayOf(*filterHid.parameters)))
    }
    ...
    fun queryCombinings(): CombiningCursor {
        return CombiningCursor(db.rawQuery("""
            SELECT c._id, c.amount_made_min, c.amount_made_max, c.percentage, c.combiner, c.alchemy,
                ${combiningItemColumns("crt", "crt")},
                ${combiningItemColumns("mat1", "mat1")},
                ${combiningItemColumns("mat2", "mat2")},
                ${combiningItemColumns("mat3", "mat3")}
            FROM combining c
                ...
                LEFT OUTER JOIN items mat3 ON c.item_3_id = mat3._id
        """, emptyArray()))
    }
    ...
    fun queryArmorUpgrades(id: Long, type: String): ArmorUpgradeCursor {
        ...
        return ArmorUpgradeCursor(db.rawQuery("""
            ...
            WHERE au.item_id = ? AND au.descendant_type = $typePredicate
        """, arrayOf(id.toString())
        ))
    }
    ...
    fun queryComponentsByArmorFamily(family: Long): ComponentCursor {
        return ComponentCursor(db.rawQuery("""
            ...
            WHERE a.family=? AND c.type = "Create"
